...
void InitDataBuf()
{
	...
	for (bufEntry = 0; bufEntry < AVAILABLE_DATA_BUFFER_ENTRY_COUNT; bufEntry++)
	{
		...
		dataBufMapPtr->dataBuf[bufEntry].full = 0;
		dataBufMapPtr->dataBuf[bufEntry].pinned = 0;
...
unsigned int FlushAllDataBuf(void) {
	unsigned int evictedEntry, reqSlotTag;
	unsigned int virtualSliceAddr;
	for (evictedEntry = dataBufLruList.tailEntry;
		evictedEntry != DATA_BUF_NONE;
		evictedEntry = dataBufMapPtr->dataBuf[evictedEntry].prevEntry) {
		if (dataBufMapPtr->dataBuf[evictedEntry].pinned == 1) {
			continue;
		}
		if (!dataBufMapPtr->dataBuf[evictedEntry].dirty) {
			// if(dataBufMapPtr->dataBuf[evictedEntry].full >= NVME_BLOCKS_PER_SLICE && CheckRowAddrDepForBuffer(virtualSliceAddr)){
			// break;
			continue;
		}
		reqSlotTag = GetFromFreeReqQ();
		virtualSliceAddr = AddrTransWrite(dataBufMapPtr->dataBuf[evictedEntry].logicalSliceAddr);
		reqPoolPtr->reqPool[reqSlotTag].reqType = REQ_TYPE_NAND;
		reqPoolPtr->reqPool[reqSlotTag].reqCode = REQ_CODE_WRITE;
		// reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag =0;
		reqPoolPtr->reqPool[reqSlotTag].logicalSliceAddr =
			dataBufMapPtr->dataBuf[evictedEntry].logicalSliceAddr;
		...
		reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry = evictedEntry;
		UpdateDataBufEntryInfoBlockingReq(evictedEntry, reqSlotTag);
		reqPoolPtr->reqPool[reqSlotTag].nandInfo.virtualSliceAddr = virtualSliceAddr;
		SelectLowLevelReqQ(reqSlotTag);
		dataBufMapPtr->dataBuf[evictedEntry].dirty = DATA_BUF_CLEAN;
		dataBufMapPtr->dataBuf[evictedEntry].full = 0;

	}
}

unsigned int AllocateDataBuf(unsigned int clean)
{
	unsigned int evictedEntry = DATA_BUF_NONE;

	// phase 1, check there is full dirty buffer and evict
	if (!clean) {
		for (evictedEntry = dataBufLruList.tailEntry;
			evictedEntry != DATA_BUF_NONE;
			evictedEntry = dataBufMapPtr->dataBuf[evictedEntry].prevEntry) {
			...
			if (dataBufMapPtr->dataBuf[evictedEntry].full >= NVME_BLOCKS_PER_SLICE) {
				// if(dataBufMapPtr->dataBuf[evictedEntry].full >= NVME_BLOCKS_PER_SLICE && CheckRowAddrDepForBuffer(virtualSliceAddr)){
				break;
			}
	...
	// phase 2, check there is clean buffer
	for (evictedEntry = dataBufLruList.tailEntry;
		evictedEntry != DATA_BUF_NONE;
		evictedEntry = dataBufMapPtr->dataBuf[evictedEntry].prevEntry) {
		if (dataBufMapPtr->dataBuf[evictedEntry].pinned == 1) {
		...
		if (dataBufMapPtr->dataBuf[evictedEntry].dirty == DATA_BUF_CLEAN) {
			break;
		}
	}
	if (evictedEntry != DATA_BUF_NONE) {
	...
	// phase 3, check there is dirty but not full buffer(which is only remain)
	if (!clean) {
		for (evictedEntry = dataBufLruList.tailEntry;
			evictedEntry != DATA_BUF_NONE;
			evictedEntry = dataBufMapPtr->dataBuf[evictedEntry].prevEntry) {
	...
	// Put evicted entry to headEntry of LRU list.
	if (dataBufMapPtr->dataBuf[evictedEntry].prevEntry != DATA_BUF_NONE) // if not head entry
	{
		if (evictedEntry != dataBufLruList.tailEntry)
		{
			//unsigned int tmp = dataBufMapPtr->dataBuf[evictedEntry].nextEntry;
		...
		else // evictedEntry == dataBufLruList.tailEntry
		{
