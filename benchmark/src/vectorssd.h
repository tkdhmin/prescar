...
namespace vectorssd {
  ...
  constexpr size_t VECTOR_DIMENSION = 3;
  ...
  struct VectorSearchReturn {
    explicit VectorSearchReturn(size_t top_k_) : top_k(top_k_), top_k_vector_id(top_k_), top_k_distance(top_k_) {}
  ...
  public:
    ...
    int Close();
    int Put(const std::string& key, const float vector[VECTOR_DIMENSION]);
    ...
    int CloseMock();
    int PutMock(const std::string& key, const float vector[VECTOR_DIMENSION]);
