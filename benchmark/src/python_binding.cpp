...
PYBIND11_MODULE(vectorssd, m) {
  ...
  py::class_<vectorssd::DB>(m, "DB")
    ...
    .def(
      "put",
      [](vectorssd::DB& self, const std::string& key, py::array_t<float> vector) {
        ...
        return self.Put(key, ptr);
      },
      py::arg("key"), py::arg("vector"), "Store vector to device with given key")
    ...
    .def(
      ...
      [](vectorssd::DB& self, const std::string& key, py::array_t<float> vector) {
        ...
        float* ptr = static_cast<float*>(buf.ptr);
        return self.PutMock(key, ptr);
      },
      py::arg("key"), py::arg("vector"), "PutMock")
