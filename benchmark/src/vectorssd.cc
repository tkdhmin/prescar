...
int DB::Open(const std::string& dev, const std::string& collectionName) {
  ...
  if (err < 0) {
       std::cout << "Device error (fstat)." << std::endl;
   
    Close();
    return -1;
  }
  if (!S_ISCHR(nvme_stat.st_mode) && !S_ISBLK(nvme_stat.st_mode)) {
        std::cout << "Device error : " << S_ISCHR(nvme_stat.st_mode) << S_ISBLK(nvme_stat.st_mode) << std::endl;
  
    Close();
    return -1;
  }

  std::cout << "[PRESCAR] Device has been connected." << std::endl;
...
int DB::Close() {
  ...
  std::cout << "[PRESCAR] Disconnected from the device." << std::endl;
  return 0;
}
...
int DB::VectorBuild() {
  ...
  printf("nvme_passthru returned: err=%d (0x%x), result=%u (0x%x)\n", err, err, result, result);
  free(data);
  ...
  if (err == 0x7C1) {
    printf("Index building job is running\n");
    // index building job is running
    return -2;
  }

  if (err == 0 && result == 1){
    printf("Build started successfully\n");
    return 0;
  }
  printf("Unexpected result: err=%d, result=%u\n", err, result);
  return -1;
}
...
int DB::VectorSearch(const float query_vector[VECTOR_DIMENSION], int top_k, VectorSearchReturn* rets) {
  ...
  // printf("rets %lu %p %p %p %p\n",rets->Size(),rets,&rets->top_k,rets->top_k_vector_id,rets->top_k_distance);
  // rets->PrintVID();
  // rets->PrintDistance();
...
int DB::PauseBuild(){
  ...
  printf("Pause Build Request: err=%d, result=%u, errno=%d (%s)\n", err, result, errno, strerror(errno));
  ...
  if (err == 0x7C1) {
    ...
    printf("Pause Build Error: err=%d, result=%u\n", err, result);
    return -1;
  }
}
...
int DB::ResumeBuild(){
  ...
  printf("Resume Build Requested: err=%d, result=%u, errno=%d (%s)\n", err, result, errno, strerror(errno));
  ...
  if (err == 0x7C1) {
    ...
    printf("Resume Build Error: err=%d, result=%u\n", err, result);
    return -1;
  }
...
int DB::VectorBuildStatus() {
  ...
  printf("Vector Build Status: err=%d, result=%u, errno=%d (%s)\n", err, result, errno, strerror(errno));
...
int DB::PutMock(const std::string& key, const float vector[VECTOR_DIMENSION], const bool delayed_compaction) {
  ...
  // }
  // std::cout << "]>" << std::endl;
  // std::cout << "delayed_compaction: " << delayed_compaction << std::endl;
  return 1;
}
